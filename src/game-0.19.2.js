...
// 0.19.2 preserves the 0.19.1 gameplay path. It removes the fine repeating
// water shading pattern, flattens the base colour, and puts visual emphasis back
// on the existing low-cost geometric waves.
...
class KDJPatchedBlob0192 extends NativeBlob {
  constructor(parts = [], options = {}) {
    ...
    if (
      ...
      if (!parts[0].includes(SWORD_CONE_30)) {
        throw new Error("0.19.2 boot failed: current 30-degree sword cone marker missing");
      }
      ...
      if (speedSnapMatches !== 1) {
        throw new Error(`0.19.2 boot failed: expected one ship speed snap marker, found ${speedSnapMatches}`);
      }
...
globalThis.Blob = KDJPatchedBlob0192;

try {
  await import("./game-0.12.1.js?v=0.12.1");
} catch (error) {
  document.querySelector("#loadingScreen")?.remove();
  console.error("KDJ2 0.19.2 boot failed", error);
