...
// Emergency 0.14.0 recovery: intentionally stop the gameplay/render patch chain at
// the last known-good 0.12.1 source. This completely excludes the 0.13.x water-state,
// water-camera, online-roster render-loop and grappling-risk modifications.
const originalNetworkStatus = window.KDJNetwork?.prototype?.status;
if (originalNetworkStatus) {
  window.KDJNetwork.prototype.status = function status0140(text, kind = "ready") {
...
class KDJPatchedBlob0140 extends NativeBlob {
  constructor(parts = [], options = {}) {
    ...
    if (
      ...
      nextParts = [patch0121(patch0120(patch0113(patch0112(patch0111(patch011(patch010(parts[0])))))))];
      ...
      globalThis.__KDJ_0140_ROLLBACK__ = true;
    }
    super(nextParts, options);
  }
}

globalThis.Blob = KDJPatchedBlob0140;

try {
  await import("./game-0.9.3.js?v=0.9.3");
} catch (error) {
  console.error("KDJ2 0.14.0 boot failed", error);
