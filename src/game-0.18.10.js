...
// 0.18.10 starts from the known-good 0.18.8 / 0.18.6 gameplay path.
// The sword cone is changed inline in this launcher so there is no separate
// 0.18.7/0.18.9 runtime patch module that can be served stale from cache.
...
class KDJPatchedBlob01810 extends NativeBlob {
  constructor(parts = [], options = {}) {
    ...
    if (
      ...
      if (!parts[0].includes(SWORD_CONE_30)) {
        throw new Error("0.18.10 boot failed: current 30-degree sword cone marker missing");
      }

      // Bake the requested ±45-degree cone into the generated combat source
      // before the established, known-good gameplay patch chain is applied.
      const combatSource = parts[0].replace(SWORD_CONE_30, SWORD_CONE_45);
      nextParts = [patch0183(patch0181(patch0180(patch0176(patch0175(patch0174(patch0173(patch0171(patch0170(patchBeta4(patchBeta3(patchBeta2(patchBeta1(patch0147(combatSource))))))))))))))];
...
globalThis.Blob = KDJPatchedBlob01810;

try {
  await import("./game-0.12.1.js?v=0.12.1");
} catch (error) {
  document.querySelector("#loadingScreen")?.remove();
  console.error("KDJ2 0.18.10 boot failed", error);
