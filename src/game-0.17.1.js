...
// 0.17.1 retains 0.17.0 gameplay and adds a compact high-rate motion stream
// plus faster guest inputs to reduce non-host movement/camera stepping.
const NativeBlob = globalThis.Blob;
let patchedFinalGame = false;

class KDJPatchedBlob0171 extends NativeBlob {
  constructor(parts = [], options = {}) {
    ...
    if (
      ...
      nextParts = [patch0171(patch0170(patchBeta4(patchBeta3(patchBeta2(patchBeta1(patch0147(parts[0])))))))];
...
globalThis.Blob = KDJPatchedBlob0171;

try {
  await import("./game-0.12.1.js?v=0.12.1");
} catch (error) {
  console.error("KDJ2 0.17.1 boot failed", error);
