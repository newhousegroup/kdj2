...
import { patchGameSource as patch0143 } from "./patch-0.14.3.js?v=0.14.3";

// 0.14.3 intentionally uses the exact 0.12.1 gameplay/render patch chain and adds
// only one tiny HUD correction: keep the British/French badge synced to the local player.
const originalNetworkStatus = window.KDJNetwork?.prototype?.status;
if (originalNetworkStatus) {
  window.KDJNetwork.prototype.status = function status0143(text, kind = "ready") {
...
class KDJPatchedBlob0143 extends NativeBlob {
  constructor(parts = [], options = {}) {
    ...
    if (
      ...
      nextParts = [patch0143(patch0121(patch0120(patch0113(patch0112(patch0111(patch011(patch010(parts[0]))))))))];
      ...
      globalThis.__KDJ_0120_PATCHED__ = true;
      globalThis.__KDJ_0121_PATCHED__ = true;
      globalThis.__KDJ_0143_PATCHED__ = true;
    }
    super(nextParts, options);
  }
}

globalThis.Blob = KDJPatchedBlob0143;

try {
  await import("./game-0.9.3.js?v=0.9.3");
} catch (error) {
  console.error("KDJ2 0.14.3 boot failed", error);
