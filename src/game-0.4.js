...
const ui = {
  ...
  crosshair: $("#crosshair"),
  leave: $("#leaveBtn"),
  britishMobility: $("#britishMobility"),
  frenchMobility: $("#frenchMobility"),
  objective: $("#objective"),
  ...
  touchGrapple: $("#touchGrapple"),
  touchRigging: $("#touchRigging")
};
...
function spawnLocal() {
  ...
  showToast(`${TEAM[localTeam].label} crew · ${lookHelp}`, 3000);
}
...
function addJib(parent, sailMat, sails) {
  ...
  sail.position.set(0.05, 6.2, -14.0);
  sail.userData.baseRotationY = sail.rotation.y;
  parent.add(sail);
  sails.push(sail);
}
...
function makeShip(team) {
  ...
  // All cloth is forward of the helm and starts well above eye level.
  // The stern around z=9 remains completely open for the captain's sightline.
  addCylinder(exterior, 0.25, 16.5, darkWood, [0, 12.45, -9.2]);
  addCylinder(exterior, 0.28, 18.5, darkWood, [0, 13.45, -1.7]);
  ...
  addSquareSail(exterior, sailMat, darkWood, { z: -1.7, y: 12.2, width: 8.4, height: 6.0, bend: 0.48 }, sails);
  addSquareSail(exterior, sailMat, darkWood, { z: -1.7, y: 17.4, width: 5.9, height: 3.5, bend: 0.34 }, sails);
  addJib(exterior, sailMat, sails);
  addRope(exterior, [[0, 20.6, -9.2], [0, 5.0, -16.8]]);
  addRope(exterior, [[0, 22.7, -1.7], [0, 20.6, -9.2]]);
  addRope(exterior, [[0, 22.7, -1.7], [0, 5.2, 13.7]]);
  addRope(exterior, [[-4.2, 5.0, -12], [0, 22.7, -1.7], [4.2, 5.0, -12]]);
...
function renderState(now = performance.now()) {
  ...
  for (const team of ["british", "french"]) {
    ...
    shipMeshes[team].exterior.visible = !inside;
    shipMeshes[team].lower.visible = inside;
  }
  ...
  if (state.phase === "cooldown") updateCooldownUi();
  else ui.victory.classList.add("hidden");
}
...
function resetRound() {
  ...
  for (const p of Object.values(state.players)) {
    ...
    processed.set(p.id, {
      interact: input.interactSeq || 0,
      grapple: input.grappleSeq || 0,
      rig: input.rigSeq || 0
    });
  }
...
function updateCamera(now) {
  ...
  const lift = p.deck === "lower" ? 0.75 : 3.0;
