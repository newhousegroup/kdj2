...
import { patchGameSource as patch0189 } from "./patch-0.18.9.js?v=0.18.9";

// 0.18.9 starts from the known-good 0.18.8/0.18.6 gameplay path and changes
// only the sword facing threshold to a 45-degree half-cone (90 degrees total).
const NativeBlob = globalThis.Blob;
let patchedFinalGame = false;

class KDJPatchedBlob0189 extends NativeBlob {
  constructor(parts = [], options = {}) {
    ...
    if (
      ...
      nextParts = [patch0189(patch0183(patch0181(patch0180(patch0176(patch0175(patch0174(patch0173(patch0171(patch0170(patchBeta4(patchBeta3(patchBeta2(patchBeta1(patch0147(parts[0]))))))))))))))))];
      ...
      globalThis.__KDJ_0189_PATCHED__ = true;
    }
    super(nextParts, options);
  }
}

globalThis.Blob = KDJPatchedBlob0189;

try {
  await import("./game-0.12.1.js?v=0.12.1");
} catch (error) {
  document.querySelector("#loadingScreen")?.remove();
  console.error("KDJ2 0.18.9 boot failed", error);
