...
// 0.19.5 keeps the full 0.19.4 visual/gameplay path and applies only the
// player-feedback fixes: correct enemy-relative compass bearing and stronger
// named-ship condition/damage feedback.
...
class KDJPatchedBlob0195 extends NativeBlob {
  constructor(parts = [], options = {}) {
    ...
    if (
      ...
      if (!parts[0].includes(SWORD_CONE_30)) {
        throw new Error("0.19.5 boot failed: current 30-degree sword cone marker missing");
      }
      ...
      if (speedSnapMatches !== 1) {
        throw new Error(`0.19.5 boot failed: expected one ship speed snap marker, found ${speedSnapMatches}`);
      }
...
globalThis.Blob = KDJPatchedBlob0195;

try {
  await import("./game-0.12.1.js?v=0.12.1");
} catch (error) {
  document.querySelector("#loadingScreen")?.remove();
  console.error("KDJ2 0.19.5 boot failed", error);
