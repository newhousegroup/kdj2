...
function showBootError(error) {
  console.error("KDJ2 0.8.0 boot failed", error);
...
async function boot() {
  ...
  function replaceRequired(pattern, replacement, label) {
    ...
    if (next === source) throw new Error(`0.8.0 patch failed: ${label}`);
    source = next;
  }

  // Preserve the single-base-module startup model when 0.7.0 itself is
  // executed from the Blob URL below.
  replaceRequired(
  ...
  // Retain the 0.7.1 cannon tuning directly in this release.
  replaceRequired(
  ...
  patch(
    ...
    'function playerRecord(id, name, team) {\\n  return { id, name, team, ship: team, deck: "upper", x: 0, z: 10, yaw: 0, role: null, cannonIndex: null, health: 100, alive: true, swordSwingAt: 0, lastSwordAt: 0, spawned: false };\\n}',
    "player health state"
  );
  patch(
    '  p.role = null;\\n  p.cannonIndex = null;\\n  p.spawned = spawn;',
...
`;
  ...
  replaceRequired(
    ...
    extraPatches + '\n  const moduleUrl = URL.createObjectURL(new Blob([source], { type: "text/javascript" }));',
    "health and sword patch injection"
  );

  source = source.replaceAll("KDJ2 0.7.0 boot failed", "KDJ2 0.8.0 boot failed");
  source = source.replaceAll("0.7.0 patch failed", "0.8.0 patch failed");
