...
// 0.18.5 keeps the complete 0.18.4 gameplay path unchanged. This release
// only replaces the narrowly scoped surrounding-UI typography.
const NativeBlob = globalThis.Blob;
let patchedFinalGame = false;

class KDJPatchedBlob0185 extends NativeBlob {
  constructor(parts = [], options = {}) {
    ...
    if (
      ...
      globalThis.__KDJ_0185_PATCHED__ = true;
    }
    super(nextParts, options);
  }
}

globalThis.Blob = KDJPatchedBlob0185;

try {
  await import("./game-0.12.1.js?v=0.12.1");
} catch (error) {
  document.querySelector("#loadingScreen")?.remove();
  console.error("KDJ2 0.18.5 boot failed", error);
