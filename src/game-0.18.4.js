...
// 0.18.4 keeps the complete 0.18.3 gameplay path unchanged. This release
// only adds narrowly scoped typography in the surrounding UI.
const NativeBlob = globalThis.Blob;
let patchedFinalGame = false;

class KDJPatchedBlob0184 extends NativeBlob {
  constructor(parts = [], options = {}) {
    ...
    if (
      ...
      globalThis.__KDJ_0184_PATCHED__ = true;
    }
    super(nextParts, options);
  }
}

globalThis.Blob = KDJPatchedBlob0184;

try {
  await import("./game-0.12.1.js?v=0.12.1");
} catch (error) {
  document.querySelector("#loadingScreen")?.remove();
  console.error("KDJ2 0.18.4 boot failed", error);
