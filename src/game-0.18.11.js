...
// 0.18.11 keeps the known-good 0.18.10 path, including the inline ±45°
// sword cone. It fixes a frame-rate-dependent Reefed startup stall by only
// snapping tiny residual ship speed to zero when the captain is not actively
// commanding W/S.
...
class KDJPatchedBlob01811 extends NativeBlob {
  constructor(parts = [], options = {}) {
    ...
    if (
      ...
      if (!parts[0].includes(SWORD_CONE_30)) {
        throw new Error("0.18.11 boot failed: current 30-degree sword cone marker missing");
      }
      ...
      if (speedSnapMatches !== 1) {
        throw new Error(`0.18.11 boot failed: expected one ship speed snap marker, found ${speedSnapMatches}`);
      }
...
globalThis.Blob = KDJPatchedBlob01811;

try {
  await import("./game-0.12.1.js?v=0.12.1");
} catch (error) {
  document.querySelector("#loadingScreen")?.remove();
  console.error("KDJ2 0.18.11 boot failed", error);
