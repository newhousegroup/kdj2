...
import { patchGameSource as patch0130 } from "./patch-0.13.0.js?v=0.13.0";
import { patchGameSource as patch0131 } from "./patch-0.13.1.js?v=0.13.1";

const originalNetworkStatus = window.KDJNetwork?.prototype?.status;
if (originalNetworkStatus) {
  window.KDJNetwork.prototype.status = function status0131(text, kind = "ready") {
...
class KDJPatchedBlob0131 extends NativeBlob {
  constructor(parts = [], options = {}) {
    ...
    if (
      ...
      nextParts = [patch0131(patch0130(patch0121(patch0120(patch0113(patch0112(patch0111(patch011(patch010(parts[0])))))))))];
      ...
      globalThis.__KDJ_0130_PATCHED__ = true;
      globalThis.__KDJ_0131_PATCHED__ = true;
    }
    super(nextParts, options);
  }
}

globalThis.Blob = KDJPatchedBlob0131;

try {
  await import("./game-0.9.3.js?v=0.9.3");
} catch (error) {
  console.error("KDJ2 0.13.1 boot failed", error);
