...
// 0.18.0 keeps the complete 0.17.6 gameplay/network/loading path and adds
// animated grappling/boarding plus cannon-driven ship destruction as a second
// host-authoritative win condition.
const NativeBlob = globalThis.Blob;
let patchedFinalGame = false;

class KDJPatchedBlob0180 extends NativeBlob {
  constructor(parts = [], options = {}) {
    ...
    if (
      ...
      nextParts = [patch0180(patch0176(patch0175(patch0174(patch0173(patch0171(patch0170(patchBeta4(patchBeta3(patchBeta2(patchBeta1(patch0147(parts[0]))))))))))))];
...
globalThis.Blob = KDJPatchedBlob0180;

try {
  await import("./game-0.12.1.js?v=0.12.1");
} catch (error) {
  document.querySelector("#loadingScreen")?.remove();
  console.error("KDJ2 0.18.0 boot failed", error);
