...
// 0.19.1 preserves the complete 0.19.0 gameplay path and only retunes the
// realistic ocean: lower reflection/glint brightness and stronger water-body
// definition, while keeping the same single lightweight ocean mesh.
...
class KDJPatchedBlob0191 extends NativeBlob {
  constructor(parts = [], options = {}) {
    ...
    if (
      ...
      if (!parts[0].includes(SWORD_CONE_30)) {
        throw new Error("0.19.1 boot failed: current 30-degree sword cone marker missing");
      }
      ...
      if (speedSnapMatches !== 1) {
        throw new Error(`0.19.1 boot failed: expected one ship speed snap marker, found ${speedSnapMatches}`);
      }
...
globalThis.Blob = KDJPatchedBlob0191;

try {
  await import("./game-0.12.1.js?v=0.12.1");
} catch (error) {
  document.querySelector("#loadingScreen")?.remove();
  console.error("KDJ2 0.19.1 boot failed", error);
