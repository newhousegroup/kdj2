...
import { patchGameSource as patch0193 } from "./patch-0.19.3.js?v=0.19.3";

// 0.19.3 preserves 0.19.2 gameplay/water and changes only presentation:
// a bright circular sun and a more realistic lightweight visual hull/deck shell.
...
class KDJPatchedBlob0193 extends NativeBlob {
  constructor(parts = [], options = {}) {
    ...
    if (
      ...
      if (!parts[0].includes(SWORD_CONE_30)) {
        throw new Error("0.19.3 boot failed: current 30-degree sword cone marker missing");
      }
      ...
      if (speedSnapMatches !== 1) {
        throw new Error(`0.19.3 boot failed: expected one ship speed snap marker, found ${speedSnapMatches}`);
      }
      ...
      finalSource = patch0193(finalSource);
      ...
      globalThis.__KDJ_0193_PATCHED__ = true;
    }
    super(nextParts, options);
  }
}

globalThis.Blob = KDJPatchedBlob0193;

try {
  await import("./game-0.12.1.js?v=0.12.1");
} catch (error) {
  document.querySelector("#loadingScreen")?.remove();
  console.error("KDJ2 0.19.3 boot failed", error);
