...
import { patchGameSource as patch0196 } from "./patch-0.19.6.js?v=0.19.6";

// 0.19.6 keeps the complete 0.19.5 gameplay/visual path. Each room code now
// deterministically owns a unique pair of ship names, shared through room state.
...
class KDJPatchedBlob0196 extends NativeBlob {
  constructor(parts = [], options = {}) {
    ...
    if (
      ...
      if (!parts[0].includes(SWORD_CONE_30)) {
        throw new Error("0.19.6 boot failed: current 30-degree sword cone marker missing");
      }
      ...
      if (speedSnapMatches !== 1) {
        throw new Error(`0.19.6 boot failed: expected one ship speed snap marker, found ${speedSnapMatches}`);
      }
      ...
      finalSource = patch0196(finalSource);
      ...
      globalThis.__KDJ_0196_PATCHED__ = true;
    }
    super(nextParts, options);
  }
}

globalThis.Blob = KDJPatchedBlob0196;

try {
  await import("./game-0.12.1.js?v=0.12.1");
} catch (error) {
  document.querySelector("#loadingScreen")?.remove();
  console.error("KDJ2 0.19.6 boot failed", error);
