...
// 0.17.3 keeps the complete 0.17.1 gameplay/network/performance path and
// applies only the reported desktop keyboard A/D direction correction.
const NativeBlob = globalThis.Blob;
let patchedFinalGame = false;

class KDJPatchedBlob0173 extends NativeBlob {
  constructor(parts = [], options = {}) {
    ...
    if (
      ...
      nextParts = [patch0173(patch0171(patch0170(patchBeta4(patchBeta3(patchBeta2(patchBeta1(patch0147(parts[0]))))))))];
...
globalThis.Blob = KDJPatchedBlob0173;

try {
  await import("./game-0.12.1.js?v=0.12.1");
} catch (error) {
  console.error("KDJ2 0.17.3 boot failed", error);
