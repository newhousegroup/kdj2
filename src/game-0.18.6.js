...
// 0.18.6 keeps the complete 0.18.5 gameplay path unchanged. This release
// only adjusts the narrowly scoped surrounding-UI typography.
const NativeBlob = globalThis.Blob;
let patchedFinalGame = false;

class KDJPatchedBlob0186 extends NativeBlob {
  constructor(parts = [], options = {}) {
    ...
    if (
      ...
      globalThis.__KDJ_0186_PATCHED__ = true;
    }
    super(nextParts, options);
  }
}

globalThis.Blob = KDJPatchedBlob0186;

try {
  await import("./game-0.12.1.js?v=0.12.1");
} catch (error) {
  document.querySelector("#loadingScreen")?.remove();
  console.error("KDJ2 0.18.6 boot failed", error);
