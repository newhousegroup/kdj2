...
// Player-facing wording only; networking behavior is unchanged.
const originalNetworkStatus = window.KDJNetwork?.prototype?.status;
if (originalNetworkStatus) {
  window.KDJNetwork.prototype.status = function status0121(text, kind = "ready") {
...
class KDJPatchedBlob0121 extends NativeBlob {
...
globalThis.Blob = KDJPatchedBlob0121;

try {
  await import("./game-0.9.3.js?v=0.9.3");
} catch (error) {
  console.error("KDJ2 0.12.1 boot failed", error);
