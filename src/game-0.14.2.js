...
import { patchGameSource as patch0142 } from "./patch-0.14.2.js?v=0.14.2";

const originalNetworkStatus = window.KDJNetwork?.prototype?.status;
if (originalNetworkStatus) {
  window.KDJNetwork.prototype.status = function status0142(text, kind = "ready") {
...
class KDJPatchedBlob0142 extends NativeBlob {
  constructor(parts = [], options = {}) {
    ...
    if (
      ...
      nextParts = [patch0142(patch0121(patch0120(patch0113(patch0112(patch0111(patch011(patch010(parts[0]))))))))];
      ...
      globalThis.__KDJ_0142_PATCHED__ = true;
    }
    super(nextParts, options);
  }
}

globalThis.Blob = KDJPatchedBlob0142;

try {
  await import("./game-0.9.3.js?v=0.9.3");
} catch (error) {
  console.error("KDJ2 0.14.2 boot failed", error);
