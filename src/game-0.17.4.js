...
// 0.17.4 retains the 0.17.3 gameplay/network/performance path and adds only
// solid player clearance at the forward bow barrier plus deck-climb animation.
const NativeBlob = globalThis.Blob;
let patchedFinalGame = false;

class KDJPatchedBlob0174 extends NativeBlob {
  constructor(parts = [], options = {}) {
    ...
    if (
      ...
      nextParts = [patch0174(patch0173(patch0171(patch0170(patchBeta4(patchBeta3(patchBeta2(patchBeta1(patch0147(parts[0])))))))))];
...
globalThis.Blob = KDJPatchedBlob0174;

try {
  await import("./game-0.12.1.js?v=0.12.1");
} catch (error) {
  console.error("KDJ2 0.17.4 boot failed", error);
