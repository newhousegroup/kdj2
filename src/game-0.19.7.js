...
// 0.19.7 is an emergency rollback to the complete 0.19.5 gameplay/visual path.
// The 0.19.6 room-specific ship-name patch is intentionally not imported or applied.
...
class KDJPatchedBlob0197 extends NativeBlob {
  constructor(parts = [], options = {}) {
    ...
    if (
      ...
      if (!parts[0].includes(SWORD_CONE_30)) {
        throw new Error("0.19.7 boot failed: current 30-degree sword cone marker missing");
      }
      ...
      if (speedSnapMatches !== 1) {
        throw new Error(`0.19.7 boot failed: expected one ship speed snap marker, found ${speedSnapMatches}`);
      }
      ...
      globalThis.__KDJ_0197_PATCHED__ = true;
    }
    super(nextParts, options);
  }
}

globalThis.Blob = KDJPatchedBlob0197;

try {
  await import("./game-0.12.1.js?v=0.12.1");
} catch (error) {
  document.querySelector("#loadingScreen")?.remove();
  console.error("KDJ2 0.19.7 boot failed", error);
