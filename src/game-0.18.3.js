...
// 0.18.3 keeps the complete 0.18.2 gameplay path and separates cannon aiming
// from the desktop A/D movement correction so left/right cannon controls match
// their physical direction without changing helm, walking, touch, or balance.
const NativeBlob = globalThis.Blob;
let patchedFinalGame = false;

class KDJPatchedBlob0183 extends NativeBlob {
...
globalThis.Blob = KDJPatchedBlob0183;

try {
  await import("./game-0.12.1.js?v=0.12.1");
} catch (error) {
  document.querySelector("#loadingScreen")?.remove();
  console.error("KDJ2 0.18.3 boot failed", error);
