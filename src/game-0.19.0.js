...
// 0.19.0 preserves the 0.18.11 gameplay path: inline ±45° sword cone and
// the Reefed startup fix. Its only new gameplay-visible change is the ocean
// rendering pass, implemented as one lightweight shader/material replacement.
...
class KDJPatchedBlob0190 extends NativeBlob {
  constructor(parts = [], options = {}) {
    ...
    if (
      ...
      if (!parts[0].includes(SWORD_CONE_30)) {
        throw new Error("0.19.0 boot failed: current 30-degree sword cone marker missing");
      }
      ...
      if (speedSnapMatches !== 1) {
        throw new Error(`0.19.0 boot failed: expected one ship speed snap marker, found ${speedSnapMatches}`);
      }
...
globalThis.Blob = KDJPatchedBlob0190;

try {
  await import("./game-0.12.1.js?v=0.12.1");
} catch (error) {
  document.querySelector("#loadingScreen")?.remove();
  console.error("KDJ2 0.19.0 boot failed", error);
