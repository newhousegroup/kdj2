...
if (originalNetworkStatus) {
  window.KDJNetwork.prototype.status = function status0130(text, kind = "ready") {
...
class KDJPatchedBlob0130 extends NativeBlob {
  constructor(parts = [], options = {}) {
    ...
    if (
      ...
      nextParts = [patch0130(patch0121(patch0120(patch0113(patch0112(patch0111(patch011(patch010(parts[0]))))))))];
...
globalThis.Blob = KDJPatchedBlob0130;

try {
  await import("./game-0.9.3.js?v=0.9.3");
} catch (error) {
  console.error("KDJ2 0.13.0 boot failed", error);
